function p = bspeval(deg,cpts,knot,u)
...
%       cpts - Control Points, matrix of size (dim,nc).
...
%for col=1:nu                                    %   for (col = 0; col < nu; col++) {
    ...
    s = findspan(nc-1, deg, u(:), knot);           %     s = findspan(nc-1, d, u[col], k);
    N = basisfun(s,u(:),deg,knot);                 %     basisfun(s, u[col], d, k, N);
                                                %
    tmp1 = s - deg + 1;                           %     tmp1 = s - d;
    %for row=1:mc                                %     for (row = 0; row < mc; row++)  {
        p = zeros (mc, nu);                               %       tmp2 = 0.0;
        for i=0:deg                               %       for (i = 0; i <= d; i++)
           p = p + repmat (N(:,i+1)', mc, 1).*cpts(:,tmp1+i);  % 	tmp2 += N[i] * ctrl[tmp1+i][row];
        end                                     %
