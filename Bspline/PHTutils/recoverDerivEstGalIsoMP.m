function [patchRef,estErrorGlob]=recoverDerivEstGalIsoMP(PHTelem, controlPts, sol0, target_rel_error, patchList, p, q, Cmat, targetScale)
...
for j=1:size(patchList,1)
    ...
    for jj=1:length(uConvPts_v)
        for ii=1:length(uConvPts_u)
            ...
            nodes = PHTelem(e_glob).nodes(1:nument);
