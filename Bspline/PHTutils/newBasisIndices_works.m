...
for j=1:length(newBasisVert)
    switch newBasisVert(j)
        case 1
            ...
            newBasisSet{2}(south) = basisCounter+(alpha+1)^2+1:basisCounter+(alpha+1)^2+(beta+1)*(p-2*alpha-1);
        ...
        case 3
            ...
            newBasisSet{4}(north) = basisCounter+(beta+1)^2+1:basisCounter+(alpha+1)^2+(beta+1)*(p-2*alpha-1);
            %increment basis counter
        ...
        case 5
            ...
            basisCounter = basisCounter + (p-2*alpha-1)*(q-2*beta-1);
            newBasisSet{3}(east) = basisCounter+1:basisCounter+(alpha+1)*(q-2*beta-1);
            newBasisSet{4}(west) = basisCounter+1:basisCounter+(alpha+1)*(q-2*beta-1);
            basisCounter = basisCounter + (alpha+1)*(q-2*beta-1);
