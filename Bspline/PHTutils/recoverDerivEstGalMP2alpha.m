function [patchRef,estErrorGlob]=recoverDerivEstGalMP2alpha(PHTelem, GIFTmesh, sol0, target_rel_error, patchList, p, q, Cmat, targetScale)
...
for j=1:size(patchList,1)
    ...
    knotV = [zeros(1,q_loc+1),1/2*ones(1,q-beta-1),ones(1,q_loc+1)];
...
%estError
local_target = targetScale/size(patchList,1)*target_rel_error^2*h1norm_glob;
