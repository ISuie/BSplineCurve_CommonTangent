function [patchRef,estErrorGlob]=recoverDerivEstGalMP2(PHTelem, GIFTmesh, sol0, target_rel_error, patchList, p, q, Cmat)
...
for j=1:size(patchList,1)
    ...
    knotU = [zeros(1,p_loc+1),1/2*ones(1,p-2),ones(1,p_loc+1)];
    knotV = [zeros(1,q_loc+1),1/2*ones(1,q-2),ones(1,q_loc+1)];
    ...
    M_int = sparse(num_int_pts, (p+2)*(q+2)); %the interpolation matrix
    ...
    for jj=1:length(uConvPts_v)
        for ii=1:length(uConvPts_u)
            ...
            e_glob = patchList(j,e);
            
            nodes_loc = elementNodeLoc(e,:);
            
            xmin = PHTelem(e_glob).vertex(1);
            xmax = PHTelem(e_glob).vertex(3);
            ymin = PHTelem(e_glob).vertex(2);
            ymax = PHTelem(e_glob).vertex(4);
            
            N = phtBasis(u_hat, v_hat, C_loc(:,:,e), p_loc, q_loc);
            
            Ce = PHTelem(e_glob).C;
            
            [ ~, dxdxi] = paramMap( GIFTmesh, u_hat, v_hat, xmin, ymin, xmax, ymax);
    ...
    for e=1:4 %for each element in the patch
        ...
        for jj=1:numGaussV
            for ii=1:numGaussU
                ...
                h1norm_glob = h1norm_glob + recovStress'*invC*recovStress*gaussWeightsU(ii)*gaussWeightsV(jj)*detJacobian;
                h1normerr = h1normerr + (recovStress-compStress)'*invC*(recovStress-compStress)*gaussWeightsU(ii)*gaussWeightsV(jj)*detJacobian;
            end
        end
...
%estError
%local_target = 1/sqrt(size(patchList,1))*target_rel_error^2*h1norm_glob
local_target = 1/size(patchList,1)*target_rel_error^2*h1norm_glob;
for j=1:size(patchList,1)
    % j
    % patchList(j,:)
    % estError(patchList(j,:))
    % sum(estError(patchList(j,:)))
    sumError = sum(estError(patchList(j,:)));
