function [ PHTelem, dimBasis, quadList ] = checkConforming( PHTelem, dimBasis, patchBoundaries, p, q, quadList )
    ...
    for boundaryIndex = 1:numBoundaries
        ...
        for indexPatch=1:length(patchAList)
            ...
            if ~isempty(indexQuadA)
                ...
                [quadList{patchA}, PHTelem{patchA}, dimBasis(patchA)] = refineMesh(quadRefA, quadListA, PHTelem{patchA}, p, q, dimBasis(patchA));
            end
            ...
            if ~isempty(indexQuadB)
                ...
                [quadList{patchB}, PHTelem{patchB}, dimBasis(patchB)] = refineMesh(quadRefB, quadListB, PHTelem{patchB}, p, q, dimBasis(patchB));
            end
        end
    end
end
