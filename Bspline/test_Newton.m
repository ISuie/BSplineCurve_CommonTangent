...
for i1 = 1 : length(cpbsA1(1,1,:))
    ...
    for i2 = 1 : length(cpbsA2(1,1,:))
        ...
        if result == 1
            ...
            for j1 = 1 : length(dknot1(:,1))
            ptdim = length(dcpb1(1,:,j1));
            [st, no_itr, norm1, psqt] = Newton_rational_modified2_vari(dcpb1(:,:,j1), p1, dknot1(j1,1), dknot1(j1,2), (dknot1(j1,1)+dknot1(j1,2))*(0.55),dcpb2(:,:,j1), p2, dknot2(j1,1), dknot2(j1,2), (dknot2(j1,1)+dknot2(j1,2))*(0.55), no_itr, error, vari);
            [alpha,beta] = kanto_condition(dcpb1(:,:,j1), p1, dknot1(j1,1), dknot1(j1,2), dcpb2(:,:,j1), p2, dknot2(j1,1), dknot2(j1,2),  vari);
